...
def go(args):
    ...
    mlflow.sklearn.save_model(
                    sk_pipe, #pipeline
                    "random_forest_dir", #output_path
                    serialization_format=mlflow.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE, #serialization format
                    signature=signature, #input and output schema
                    input_example=X_val.iloc[:5] #examples
                    )
    ######################################
